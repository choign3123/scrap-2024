...
/**
 * 로그인&로그아웃 상태에서만 사용 가능한 API를 걸러내기 위함.
 */
@RequiredArgsConstructor
@Slf4j
@Component
public class LoginAndLogoutStatusInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        log.info("login & logout 인터셉터-member: {}", member.getName());

        // 로그인 상태인지 확인
        switch (member.getMemberLog().getLoginStatus()){
            case ACTIVE, LOGOUT -> {return true;}
